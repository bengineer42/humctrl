...
nested inside a flyball checkout the same way (`flyball-runner rig.yaml` from this directory),
...
```sh
uv run flyball-runner rig.yaml
```
...
| | |
| --- | --- |
| `rig.yaml` / `sim.yaml` | the real rig, and the overlay that replaces hardware with a simulation |
